      ...
      <div className="px-4 py-3 border-b border-white/10 flex flex-col gap-3">
        ...
        <form onSubmit={handleSearch} className="relative group w-full">
          ...
          <div className="relative flex items-center bg-[#0A0F14] border border-white/10 rounded px-3 py-1.5 shadow-lg">
            <Search className="text-gray-400 group-focus-within:text-purple-400 transition-colors mr-2" size={14} />
            <input 
              type="text"
              value={searchInput}
              onChange={(e) => setSearchInput(e.target.value)}
              placeholder="Search ticker (e.g., TSLA, MSFT)..."
              className="w-full bg-transparent border-none outline-none text-white font-mono text-xs placeholder:text-gray-500"
            />
          </div>
        </form>
      </div>
