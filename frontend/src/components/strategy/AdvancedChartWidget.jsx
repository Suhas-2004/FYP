  ...
  return (
    <div className="w-full h-full min-h-[600px] flex flex-col bg-[#131722] rounded-xl overflow-hidden border border-[#2A2E39]">
      <div className="px-4 py-3 border-b border-[#2A2E39] flex flex-col gap-3">
          ...
          <div className="relative z-50">
            <form onSubmit={handleAddSubmit} className="flex items-center gap-2">
              <input 
                ...
                className="bg-[#2A2E39] text-white text-xs px-3 py-1.5 rounded outline-none border border-transparent focus:border-brand-cyan transition-colors w-40"
              />
              <button type="submit" className="bg-brand-cyan hover:bg-brand-cyan/90 text-[#02040A] p-1.5 rounded transition-colors flex items-center justify-center">
              ...
              <div className="absolute top-full mt-1 right-0 w-64 max-h-60 overflow-y-auto custom-scrollbar bg-[#1E222D] border border-[#2A2E39] rounded-md shadow-xl py-1">
                {filteredStocks.map(sym => (
                  ...
                  >
                    ...
                    <span className="text-gray-400 truncate ml-2 text-[10px]">{COMPANY_NAMES[sym] || 'Stock'}</span>
                  </button>
                ))}
              </div>
            )}
        ...
        {watchlist.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {watchlist.map(sym => (
              <div key={sym} className="flex items-center gap-1.5 bg-[#2A2E39] px-2 py-1 rounded text-xs text-gray-400 group hover:text-white transition-colors">
                <span className="font-medium tracking-wide">{sym}</span>
                <button 
                  onClick={() => handleRemove(sym)} 
                  className="opacity-50 group-hover:opacity-100 hover:text-red-500 transition-colors"
                >
                  <X className="w-3 h-3" />
                </button>
              </div>
      ...
      <div className="flex-1 w-full relative">
        {/* Loading Spinner */}
        <div className="absolute inset-0 flex items-center justify-center bg-[#131722] z-0">
          <div className="flex flex-col items-center gap-3">
            <div className="w-8 h-8 border-4 border-brand-cyan/20 border-t-brand-cyan rounded-full animate-spin"></div>
            <span className="text-xs text-brand-cyan font-mono animate-pulse">Loading Chart Data...</span>
          </div>
        </div>
        
        <div className="tradingview-widget-container w-full h-full absolute inset-0 z-10" ref={container}>
          <div className="tradingview-widget-container__widget w-full h-full bg-transparent"></div>
        </div>
      </div>
    </div>
  );
